...
def wr_csv(
    ...
    if suffix == "coe":
        ...
        str_csv += f"{RIT.RITH.get_dw(0):08x},\n"
        str_csv += f"{RIT.RITH.get_dw(1):08x},\n"
        str_csv += f"{RIT.RITH.get_dw(2):08x},\n"
        str_csv += f"{RIT.RITH.get_dw(3):08x},\n"

        for RITE in RIT.RITE:
            str_csv += f"{RITE.get_dw(0):08x},\n"
            str_csv += f"{RITE.get_dw(1):08x},\n"
            str_csv += f"{RITE.get_dw(2):08x},\n"
            str_csv += f"{RITE.get_dw(3):08x},\n"
        ...
        dw_idx = 0
        str_csv += f"{dw_idx:04x}:{RIT.RITH.get_dw(0):08x};\n"

        dw_idx += 1
        str_csv += f"{dw_idx:04x}:{RIT.RITH.get_dw(1):08x};\n"

        dw_idx += 1
        str_csv += f"{dw_idx:04x}:{RIT.RITH.get_dw(2):08x};\n"

        dw_idx += 1
        str_csv += f"{dw_idx:04x}:{RIT.RITH.get_dw(3):08x};\n"

        for RITE in RIT.RITE:
            dw_idx += 1
            str_csv += f"{dw_idx:04x}:{RITE.get_dw(0):08x};\n"

            dw_idx += 1
            str_csv += f"{dw_idx:04x}:{RITE.get_dw(1):08x};\n"

            dw_idx += 1
            str_csv += f"{dw_idx:04x}:{RITE.get_dw(2):08x};\n"

            dw_idx += 1
            str_csv += f"{dw_idx:04x}:{RITE.get_dw(3):08x};\n"
    ...
    if SIM == True:
        ...
        file.write(f"{RIT.RITH.get_dw(0):08x}\n")
        file.write(f"{RIT.RITH.get_dw(1):08x}\n")
        file.write(f"{RIT.RITH.get_dw(2):08x}\n")
        file.write(f"{RIT.RITH.get_dw(3):08x}\n")

        for RITE in RIT.RITE:
            file.write(f"{RITE.get_dw(0):08x}\n")
            file.write(f"{RITE.get_dw(1):08x}\n")
            file.write(f"{RITE.get_dw(2):08x}\n")
            file.write(f"{RITE.get_dw(3):08x}\n")
