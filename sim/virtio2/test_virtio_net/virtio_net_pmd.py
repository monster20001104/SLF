...
from virtio_net_defines import Cfg, VirtioVq, TestType, VirtioStatus, Mbufs, VirtioErrCode, VirtioCtrlRegOffset
...
class Virt:
    ...
    async def stop(self, vq, forced_shutdown=0) -> None:
        ...
        virtq = self.virtq[vq]
        self.log.debug(f"vq: {vq_str} stop start")
    ...
    async def tx_check_result(self) -> None:
        ...
        while True:
            ...
            for i in range(pkt_num):
                ...
                # self.log.error(f"check total_len {total_len}")
                if total_len == 0:
                    continue
                T = 0
    ...
    async def worker_clean(self, vq: int) -> None:
        ...
        if typ == TestType.NETTX:
            ...
            for i in range(avail_num):
                ...
                self.log.info(f"{vq_str} seq_num {info.seq_num} drop")
                if info.qos_en:
                    net_tx.qos_update_queues[qid].get_nowait()
            ...
            for vq, virtq in self.virtq.items():
                ...
                if virtq.status == VirtioStatus.DOING:
                    ...
                    if virtq.status == VirtioStatus.DOING and status == VirtioStatus.IDLE:
                        ...
                        virtq.stop_event.set()
                        # self.log.error(err_info)
                        if err_info not in virtq.err_info_option and err_info is not virtq.err_info:
                            vq_str = VirtioVq.vq2str(vq)
