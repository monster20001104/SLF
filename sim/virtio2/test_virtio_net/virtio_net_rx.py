...
class VirtioNetRx:
    def __init__(self, tb: TB):
        ...
        self.BKT_FF_DEPTH = self.dut.u_virtio_top.PKT_ID_NUM.value
    ...
    def start(self, qid_list):
        ...
        self._process_drop_data_cr = cocotb.start_soon(self._process_drop_data())
    ...
    async def join(self, qid_list):
        ...
        await self._process_drop_data_cr.join()
    ...
    async def _process_drop(self):
        ...
        while True:
            if self.dut.u_virtio_top.u_virtio_rx_buf_top.u_virtio_rx_buf_drop.drop_info_rd_rsp_vld.value == 1:
                ...
                if 0 < self.cfg.global_rx_random_sel < 8 and bkt_ff_usedw < self.BKT_FF_DEPTH/4:
                    if (drop_random & (2 ** (self.cfg.global_rx_random_sel + 1) - 1)) == 0:
                        info.rand_drop = 1

                if 0 < self.cfg.global_rx_time_sel < 8 and bkt_ff_usedw < self.BKT_FF_DEPTH/2 and send_cnt != rsv_cnt:
    ...
    async def _process_qos_req(self):
        while True:
            ...
            # self.log.error("get_qos req")
            # self.log.error(f"qos en {info.qos_en}")
            if info.qos_en:
                self.log.error("get_qos req en")
    ...
    async def _process_qos_rsp(self):
        while True:
            ...
            if info.qos_en == 1:
                ...
                if info.qos_ok == 0:
                    info.qos_drop = 1
                self.interfaces.rx_qos.query_rsp_if.send_nowait(rsp_trans)
            # self.log.error("get_qos rsp_put")
    ...
    async def _process_drop_data(self):
        await RisingEdge(self.dut.clk)
        while True:
            if self.dut.u_virtio_top.u_virtio_rx_buf_top.u_virtio_rx_buf_drop.drop_qos_vld.value == 1 and self.dut.u_virtio_top.u_virtio_rx_buf_top.u_virtio_rx_buf_drop.drop_qos_rdy.value == 1:
