...
#(
    ...
    parameter HOST_INTERFACE_NUM_WR = 4,
    ...
    initial begin
        ...
        $fsdbDumpvars(0, virtio_net_tb, "+all");
        $fsdbDumpMDA();
    end
    ...
    clear_x #(
        .DW($bits(desc_t))
    ) u_dma_rd_req_desc_clearx (
        .in (dma_rd_req_if.desc),
        .out(dma_rd_req_desc)
    );
    ...
    clear_x #(
        .DW($bits(desc_t))
    ) u_dma_wr_req_desc_clearx (
        .in (dma_wr_req_if.desc),
        .out(dma_wr_req_desc)
    );
    ...
    virtio_top u_virtio_top (
        ...
        .idx_eng_dma_wr_req_if            (host_arbt_slave_wr_req_if[3]),
        .idx_eng_dma_wr_rsp_if            (host_arbt_slave_wr_rsp_if[3]),
