...
from virtio_net_defines import Cfg, VirtioStatus, VirtioVq, VirtqDesc, TestType, VirtqUsedElement, VirtioErrCode
...
class VirtQ:
    ...
    async def start(self, avail_idx: int = 0) -> None:
        while self.status != VirtioStatus.IDLE:
            await Timer(1, "us")
        ...
        self.err_info_list = self.err_info_gen_list(min_err_num=10, max_err_num=20)
        self.idx_defect = None

        if self.cfg.msix_time == None:
            msix_time = random.randint(0, 7)
        else:
            msix_time = self.cfg.msix_time
        if self.cfg.msix_threshold == None:
            msix_threshold = random.randint(0, 7)
        else:
            msix_threshold = self.cfg.msix_threshold
        self.log.info(
