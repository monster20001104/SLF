...
class TB(object):
    ...
    def pmd_worker(self, typ: TestType, qid_list: List[int]) -> None:
        ...
        if typ == TestType.BLK:
            cocotb.start_soon(self.virtio_pmd.blk_check_result())
            pass
        for qid in qid_list:
            vq = VirtioVq.qid2vq(qid=qid, typ=typ)
            if typ == TestType.NETTX:
                self.net_worker_cr[vq] = cocotb.start_soon(self.virtio_pmd.tx_worker(vq))
            if typ == TestType.NETRX:
                self.net_worker_cr[vq] = cocotb.start_soon(self.virtio_pmd.rx_worker(vq))
            if typ == TestType.BLK:
                self.net_worker_cr[vq] = cocotb.start_soon(self.virtio_pmd.blk_worker(vq))

    async def join_virtio(self, typ, qid_list):
        for qid in qid_list:
            vq = VirtioVq.qid2vq(qid, typ)
            await self.worker_cr[vq].join()
            await self.net_worker_cr[vq].join()
        if typ == TestType.NETRX:
    ...
    def stop_worker(self, typ: TestType, qid_list: List[int]) -> None:
        async def _stop_worker(tb: TB, vq: VirtioVq):
            virtq = tb.virtio_pmd.virtq[vq]
            # qid, typ = VirtioVq.vq2qid(vq)
            while self.cfg.restart_en and not virtq.finished:
                # random_time = 40
                if typ == TestType.BLK:
                ...
                if not virtq.stop_event.is_set():
                    virtq.stop_event.set(random.randint(0, 1))
                # virtq.stop_event.set(0)
                # self.log.error(f"virtq: {VirtioVq.vq2str(vq)} random_time {random_time}  stop_event 0")
                # break
...
async def run_test(dut, cfg: Optional[Cfg] = None, idle_inserter=None, backpressure_inserter=None):
    ...
    if cfg.blk_en:
        tb.setup_virtio(TestType.BLK, qid_list)
    await Timer(100, "us")
    if cfg.tx_en:
    ...
    if cfg.blk_en:
        tb.virtio_net.blk.start(qid_list)
    ...
    if cfg.blk_en:
        tb.pmd_worker(TestType.BLK, qid_list)
    ...
    if cfg.blk_en:
        tb.stop_worker(TestType.BLK, qid_list)
    ...
    if cfg.blk_en:
        await tb.join_virtio(TestType.BLK, qid_list)
...
if cocotb.SIM_NAME:
    for test in [run_test]:
        ...
        # factory.add_option("cfg", [Test_1Q_pps_cfg])
