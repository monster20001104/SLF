...
class TB(object):
    ...
    async def cycle_reset(self):
        ...
        len_width = int(len(self.dut.sch_len)/sch_num)
        ...
        class SchLenObj(Packet):
            name = 'sch len'
            fields_desc = [
                BitField("port{}".format(i), 2, len_width) for i in range(sch_num)
            ]
        schLen = SchLenObj()
        self.dut.sch_len.value = int.from_bytes(schLen.build(), byteorder="little")
...
async def run_test_sch(dut):
    ...
    max_seq = 20000 * len(dut.sch_grant)
    ...
    async def sch_rsp():
        ...
        for i in range(1, len(dut.sch_grant)):
            assert abs(cnts[i-1]/i - cnts[i]/(i+1)) < 1
